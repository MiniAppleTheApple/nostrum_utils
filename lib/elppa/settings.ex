defmodule Elppa.Settings do
  ...
  def valid_style_code(_x), do: []
  
  def valid_link_button(%{"style" => 5, "url" => _url}), do: [] 
  def valid_link_button(%{"style" => 5}), do: [:invalid_link_button]
  def valid_link_button(_button), do: []
  ...
  def string_length_passed_the_limit(_key, _value, _limit), do: [] 

  def not_a_color(color) when is_integer(color), do: if color > 0xffffff or color < 0, do: [:not_a_color], else: []
  def not_a_color(_color), do: []

  
  def not_a_boolean(_attribute, x) when is_boolean(x) or is_nil(x), do: []
  def not_a_boolean(attribute, _x), do: [{:not_a_boolean, attribute}]

  def not_a_string(_attribute, x) when is_binary(x) or is_nil(x), do: []
  def not_a_string(attribute, _attribute), do: [{:not_a_string, attribute}]

  def not_a_list(_attribute, x) when is_list(x) or is_nil(x), do: []
  def not_a_list(attribute, _x), do: [{:not_a_list, attribute}]
  ...
  def not_a_link(_attribute, _link), do: []
  
  def not_a_int(_attribute, x) when is_integer(x) or is_nil(x), do: []
  def not_a_int(attribute, _x), do: [{:not_a_int, attribute}]

  def not_a_map(_attribute, x) when is_map(x) or is_nil(x), do: []
  def not_a_map(attribute, _x), do: [{:not_a_map, attribute}]

  def valid_button({button, index}) when is_map(button) do 
    valid_style_code(button["style"])
    ++ valid_link_button(button)
    ++ not_a_string("buttons.#{index}.label", button["label"])
    ++ not_a_int("buttons.#{index}.style", button["style"])
    ++ required(button, ["label", "style"], "buttons.#{index}.")
  end
  ...
  def from_map_to_embed(embed) when is_map(embed) do
    ...
    %Embed{
      new | thumbnail: struct(Embed.Image, new.thumbnail),
      image: struct(Embed.Image, new.image),
      fields: new.fields |> Enum.map(&struct(Embed.Field, &1)),
      footer: struct(Embed.Footer, new.footer)
    }
  end
  ...
  def from_map(%{"embed" => embed, "buttons" => buttons}) when is_map(embed) do
    %__MODULE__{
      embed: from_map_to_embed(embed),
      buttons: Enum.map(buttons, fn x -> Button.button(map_key_to_atom(x)) end)
    }
  end
